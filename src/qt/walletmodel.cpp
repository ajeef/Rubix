...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        // reject rubix fee
        unsigned int rubixfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        rubixfee = ((MIN_RELAY_TX_FEE * rubixfee) * 10000);
        if (nFeeRequired > rubixfee){
            LogPrintf("nFeeRequired: %d -- rubixFee: %d\n", nFeeRequired, rubixfee);
            return rubixFee;
        }
    }
