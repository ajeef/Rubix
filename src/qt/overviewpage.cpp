...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeRuBiXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRuBiXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeRuBiXAmount = strAnonymizeRuBiXAmount.remove(strAnonymizeRuBiXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRuBiXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeRuBiXAmount*COIN) nMaxToAnonymize = nAnonymizeRuBiXAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeRuBiXAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeRuBiXAmount));
        strAnonymizeRuBiXAmount = strAnonymizeRuBiXAmount.remove(strAnonymizeRuBiXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRuBiXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
